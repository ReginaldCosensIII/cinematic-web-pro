                ...
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 ${
                    activeCategory === cat
                      ...
                      : `glass-effect ${isDark ? 'text-wdp-text-secondary hover:text-wdp-text' : 'text-wdp-text-secondary hover:text-wdp-text'}`
                  }`}>
                  {cat}
                </button>
              ))}
                  ...
                  <div className="relative glass-effect rounded-2xl overflow-hidden transition-all duration-500">
                    <div className={`relative overflow-hidden ${index === 0 && filtered.length > 2 ? 'aspect-[21/9]' : 'aspect-video'}`}>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
                        ...
                        <p className="text-gray-300/80 text-sm leading-relaxed max-w-lg line-clamp-2">{project.description}</p>
        ...
        {selectedProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 40, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 40, scale: 0.97 }}
              ...
              className={`relative z-10 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl border ${isDark ? 'glass-effect border-webdev-glass-border' : 'bg-white border-gray-200'}`}
              onClick={e => e.stopPropagation()}>
              <button onClick={() => setSelectedProject(null)}
                className={`absolute top-4 right-4 z-20 w-10 h-10 rounded-full border flex items-center justify-center transition-colors ${isDark ? 'bg-webdev-darker-gray/90 border-webdev-glass-border text-wdp-text-secondary hover:text-white' : 'bg-white/90 border-gray-200 text-gray-500 hover:text-gray-800'}`}>
                <X className="w-5 h-5" />
              </button>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProject.technologies.map(tech => (
                      <span key={tech} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${isDark ? 'bg-white/5 text-wdp-text border border-white/10' : 'bg-gray-100 text-gray-700 border border-gray-200'}`}>{tech}</span>
                    ))}
                  </div>
                </div>
