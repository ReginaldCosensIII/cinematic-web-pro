  ...
  <div className="group relative glass-effect rounded-xl overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-webdev-gradient-blue/10">
    <div className="relative w-full aspect-video overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 z-10">
        <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">{project.client}</p>
        <h3 className="text-2xl md:text-3xl font-bold text-white mb-2">{project.title}</h3>
        <p className="text-gray-300 text-sm md:text-base leading-relaxed max-w-xl mb-4">{project.description}</p>
        <div className="flex flex-wrap gap-2 mb-5">
          {project.technologies.map((tech) => (
            <span key={tech} className="px-2.5 py-1 rounded-md text-xs font-medium bg-white/10 text-gray-200 backdrop-blur-sm border border-white/10">
              {tech}
            </span>
          ))}
        </div>
    ...
    <section id="featuredwork" className="relative py-16 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <button
            onClick={prev}
            className={`absolute -left-2 md:-left-16 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full border text-wdp-text-secondary hover:text-wdp-text flex items-center justify-center transition-colors backdrop-blur-sm ${isDark ? 'bg-webdev-darker-gray/90 border-webdev-glass-border' : 'bg-white/90 border-gray-200'}`}
          ...
          <button
            onClick={next}
            className={`absolute -right-2 md:-right-16 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full border text-wdp-text-secondary hover:text-wdp-text flex items-center justify-center transition-colors backdrop-blur-sm ${isDark ? 'bg-webdev-darker-gray/90 border-webdev-glass-border' : 'bg-white/90 border-gray-200'}`}
